export const BASE_API_URL = 'https://v2.api.noroff.dev';
export const AUTH_REGISTER_URL = `${BASE_API_URL}/auth/register`;
export const AUTH_LOGIN_URL = `${BASE_API_URL}/auth/login`; 
export const AUTH_BASE = `${BASE_API_URL}/auth`;
export const SOCIAL_BASE = `${BASE_API_URL}/social`;
export const NOROFF_API_KEY = "1e3b241b-9285-4887-9804-427ae10ce544";
export const POSTS_URL = `${BASE_API_URL}/social/posts`;
export const PROFILES_BASE = `${BASE_API_URL}/social/profiles`;

export const STORAGE_KEYS = {
    token: "accessToken",
    profile: "ns_profile",
    userName: "currentUserName",
};
